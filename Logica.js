...
// ({ rol, usuario, token }, en sessionStorage — no localStorage, para que
// la sesión no sobreviva a cerrar el navegador). Login exitoso aquí
// redirige directo a admin.html, que reconoce esa sesión sin pedir
// credenciales de nuevo.
const SESION_ADMIN_KEY = 'bxua_sesion';

// Las credenciales reales YA NO viven aquí ni en ningún archivo del
// cliente: se validan en Apps Script (apps-script/AuthServidor.gs,
// PropertiesService) vía una llamada a doPost con action:'login'. Este
// archivo solo calcula el hash SHA-256 de la contraseña escrita (para no
// mandar texto plano por la red) y envía ese hash al servidor — nunca
// compara nada localmente ni guarda una lista de usuarios/contraseñas.
    ...
    if (rolElegido === 'admin') {
      ...
      try {
        const res = await fetch(APPS_SCRIPT_URL, {
          method: 'POST',
          body: JSON.stringify({ action: 'login', usuario: usuario, claveHash: claveHash }),
        });
        const resultado = await res.json();

        if (!resultado || !resultado.ok) {
          err.textContent = (resultado && resultado.error) || 'Usuario o contraseña incorrectos';
          err.classList.add('vis');
          return;
        }

        err.classList.remove('vis');
        sessionStorage.setItem(SESION_ADMIN_KEY, JSON.stringify({
          rol: resultado.rol, usuario: resultado.usuario, token: resultado.token,
        }));
        window.location.href = 'admin.html';
      } catch (e) {
        err.textContent = 'No se pudo conectar con el servidor. Verifica tu conexión e intenta de nuevo.';
        err.classList.add('vis');
      }
      return;
    }
